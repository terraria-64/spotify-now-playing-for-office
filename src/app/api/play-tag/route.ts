...
// Last.fm からタグのトップアーティストを取得する
async function getTagTopArtists(tag: string): Promise<string[]> {
...
// タグに合ったアーティストをコンテキストとして再生する
export async function POST(request: Request) {
  const { tag } = await request.json() as { tag: string };

  if (!tag) {
    return Response.json({ error: "tag は必須です" }, { status: 400 });
  }

  // タグのトップアーティストを取得
  let artists: string[];
  try {
    artists = await getTagTopArtists(tag);
  } catch (e) {
